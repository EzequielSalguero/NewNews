...
public class ComprobarUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Usuario usu = ctrUsuario.findUsuario(usuario);
        if (usu != null && usu.getPassword().equals(pwd)) {
            HttpSession session = request.getSession();
            session.setAttribute("usu", usu);
            if (usu.getRol().equals("Redactor")) {
                session.setAttribute("redactor", "si");
            }
            if (usu.getRol().equals("Administrador")) {
                session.setAttribute("administrador", "si");
            }
            //acceso="Si";

        } else {
            request.setAttribute("mensaje", "Compruebe usuario y contraseña");
            //acceso="No";
        }
